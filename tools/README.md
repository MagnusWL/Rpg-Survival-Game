...
> **Rescued 23 July 2026.** Magnus's cleanup ("Remove old project and move
> defold-port into main folder") lifted the web project out of git, and this
> workshop lived inside it -- so all of it went with the tide. Everything here
> is restored verbatim from commit `9b85e8d`, the last one before the move.
>
> **The mills come in two eras:**
>
> - **Defold era, live and repointed** -- `record-server.mjs`,
>   `record-tear.html`, `record-intro.html`, `pack-tear.mjs`, `pack-intro.mjs`.
>   They wrote the tear film and the three intro loops now in the game, and
>   their output paths were moved from `defold-port/assets` and
>   `defold-port/main` to plain `assets/` and `main/` to match the new root.
>   Proven after the move: re-running `pack-intro.mjs` produced all 241 frames
>   byte-identical to the committed ones.
> - **Web era, kept but not repointed** -- everything else (`build-sprites`,
>   `build-sounds`, `build-music`, the eq/rain tuners, the check/verify tools).
>   They read from `Raw_Assets/` and write to a web `assets/` tree that no
>   longer exists in this repo. Nothing is deleted; whichever one is needed
>   first gets repointed then, against a real need.
>
> Paths in the rest of this file are from the web era. The art is still in
> `Raw_Assets/` exactly as described.
